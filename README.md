...
* Перезагрузите YAML конфигурацию через "Настройки" > "Сервер" (не забудьте включить "Расширенный режим" в профиле пользователя), в "Настройки" > "Журнал сервера" появятся ошибки если что-то пошло не так
