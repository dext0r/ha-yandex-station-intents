...
Компонент генерирует события или запускает действия в Home Assistant на конкретные голосовые команды, адресованные Яндекс.Станции (или колонкам с Алисой).
Как это работает: вы говорите "Алиса, сделай действие" и в Home Assistant появляется событие с текстом "сделай действие", именем колонки и комнаты. Такие события очень удобно использовать в автоматизациях. Или вместо автоматизации можно сразу выполнить нужное действие.
...
* Возможно удалить все сценарии из УДЯ одним вызовом действия `yandex_station_intents.clear_scenarios`
...
* Перезагрузите YAML конфигурацию через `Инструменты разработчика` > `YAML`, в `Настройки` > `Система` > `Журнал сервера` появятся ошибки если что-то пошло не так

### Список фраз (интенты)

Интеграция работает с заранее сформированным списком фраз, на основе которых будут автоматически созданы сценарии в [УДЯ](https://yandex.ru/quasar/iot). Активационные фразы могут содержать **только** кириллицу, цифры и пробелы. Максимальное количество сценариев в УДЯ - 200.
...
В данном случае интеграция автоматически создаст в УДЯ девять сценариев, каждый из которых начинается с символов `---`. **Не удаляйте эти символы и не модифицируйте никак название!** По ним компонент понимает, что это его сценарий и в случае необходимости синхронизирует/удалит его.

Дополнительные параметры `say_phrase`, `extra_phrases`, `execute_command`, `account`, `action` являются необязательными и могут использоваться в любых вариациях.
...
9. Срабатывает от `Алиса, запусти отсчёт`, запустит действие `timer.start` на таймере `timer.countdown`. В блоке `action` задаются параметры действия, которые можно сформировать через `Настройки` -> `Инструменты разработчика` -> `Действия`.
...
При синхронизации обновляются **только изменённые** сценарии: если сценарий в УДЯ уже соответствует интенту, он пропускается. Это уменьшает количество запросов к УДЯ и ускоряет синхронизацию.

Запустить синхронизацию вручную можно действием `yandex_station_intents.sync`. Опция `full` управляет режимом:

* `full: false` (по умолчанию) — обычная синхронизация, обновляются только изменённые сценарии;
* `full: true` — принудительная полная синхронизация, обновляются **все** сценарии, в том числе неизменённые (например, если по какой-то причине компонент считает сценарии идентичным, хотя это не так).

Пример вызова через `Инструменты разработчика` > `Действия`:

```yaml
actions: yandex_station_intents.sync
data:
  full: true
```

Важно: `yandex_station_intents.sync` синхронизирует уже загруженную конфигурацию и не учитывает изменения в YAML конфигурации.
...
2. Удалите сценарии, которые были созданы компонентом Yandex.Station. Если в УДЯ нет вручную созданных сценариев - воспользуйтесь действием [`yandex_station_intents.clear_scenarios`](#удаление-всех-сценариев), в противном случае - удалите сценарии вручную.
...
Компонент позволяет удалить **абсолютно все** сценарии из УДЯ через действие `yandex_station_intents.clear_scenarios`. Будут удалены в том числе и сценарии, созданные вручную.

Для удаления вызовите действие через `Инструменты разработчика` > `Действия`:

```yaml
action: yandex_station_intents.clear_scenarios
data:
