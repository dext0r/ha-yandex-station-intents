...
HACS > Интеграции > 3 точки (правый верхний угол) > Пользовательские репозитории > URL: `dext0r/ha-yandex-station-intents`, Категория: `Интеграция` > Добавить > подождать > выбрать `Yandex.Station Intents` > Установить
...
## Настройка
* Добавьте интеграцию в Home Assistant: Настройки > Интеграции > Добавить интеграцию > `Yandex.Station Intents` (если интеграции нет в списке - обновите страницу)
...
* Перезагрузите настройки интеграции через "Настройки" > "Сервер" (не забудьте включить "Расширенный режим" в профиле пользователя), в "Настройки" > "Журнал сервера" появятся ошибки если что-то пошло не так
...
### Режим синхронизации
По умолчанию сценарии синхронизируются с УДЯ автоматически при запуске Home Assistant. Это поведение можно отключить добавлением в конфигурацию опции `autosync: false`. После этого сценарии будут синхронизированы **только** при перезагрузке конфигурации компонента со страницы "Настройки" > "Сервер" или через сервис `yandex_station_intents.reload`.
...
5. Перезагрузите конфигурацию `Yandex.Station Intents` сценарии будут созданы автоматически
